...
//escapa la comilla simple para que no rompa el query (ej: "O'BRIEN S.A.")
const txt = (v)=> (v===undefined || v===null) ? '' : v.toString().replace(/'/g,"''");
...
//CRUD EMPRESAS_CONTA
//DOCS = documentos de ORDERS que ya usan la empresa, sirve para bloquear el borrado
router.post("/select_empresas_conta_listado", async(req,res)=>{

        let qry = `
        SELECT  EMPRESAS_CONTA.CODEMP, 
                ISNULL(EMPRESAS_CONTA.NIT,'') AS NIT, 
                ISNULL(EMPRESAS_CONTA.RAZON_SOCIAL,'') AS RAZON_SOCIAL, 
                ISNULL(EMPRESAS_CONTA.NOMBRE_COMERCIAL,'') AS NOMBRE_COMERCIAL, 
                ISNULL(EMPRESAS_CONTA.DIRECCION,'') AS DIRECCION,
                (SELECT COUNT(*) FROM ORDERS 
                        WHERE ORDERS.CODEMP = EMPRESAS_CONTA.CODEMP) AS DOCS
        FROM EMPRESAS_CONTA
        ORDER BY EMPRESAS_CONTA.RAZON_SOCIAL;
        `
...
router.post("/insert_empresa_conta", async(req,res)=>{

        const {nit,razon_social,nombre_comercial,direccion} = req.body;

        //CODEMP es IDENTITY, lo genera SQL Server
        let qry = `
        INSERT INTO EMPRESAS_CONTA (NIT,RAZON_SOCIAL,NOMBRE_COMERCIAL,DIRECCION)
        SELECT  '${txt(nit)}' AS NIT, 
                '${txt(razon_social)}' AS RAZON_SOCIAL, 
                '${txt(nombre_comercial)}' AS NOMBRE_COMERCIAL, 
                '${txt(direccion)}' AS DIRECCION;
        `
...
router.post("/edit_empresa_conta", async(req,res)=>{

        const {codemp,nit,razon_social,nombre_comercial,direccion} = req.body;

        let qry = `
        UPDATE EMPRESAS_CONTA SET 
                NIT='${txt(nit)}', 
                RAZON_SOCIAL='${txt(razon_social)}', 
                NOMBRE_COMERCIAL='${txt(nombre_comercial)}', 
...
router.post("/delete_empresa_conta", async(req,res)=>{

        const {codemp} = req.body;

        //no se borra si ya hay documentos apuntando a la empresa, 
        //quedarian huerfanos y se rompen los reportes de contabilidad
        let qry = `
        IF EXISTS (SELECT 1 FROM ORDERS WHERE CODEMP = ${Number(codemp)})
                SELECT 'en_uso' AS RESULTADO;
        ELSE
        BEGIN
                DELETE FROM EMPRESAS_CONTA WHERE CODEMP = ${Number(codemp)};
